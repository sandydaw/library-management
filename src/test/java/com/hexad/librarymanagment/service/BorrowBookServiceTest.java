...
public class BorrowBookServiceTest {
    ...
    public void borrowBook_shouldAddBookToUser() {
        given(bookRepository.findByBookId(BOOK_ID)).willReturn(new Book(3, "test book name3", "test publication3", 2));
    ...
    public void borrowBook_shouldThrowException() {
        books.add(new Book(2, "test book name2", "test publication2", 1));
        books.add(new Book(3, "test book name3", "test publication3", 3));
        given(bookRepository.findByBookId(BOOK_ID)).willReturn(new Book(3, "Test Book Name3", "Test Publication3", 1));
        given(borrowRepository.findByUserId(USER_ID)).willReturn(new User(USER_ID, "test user name", books));
    ...
    public void borrowBook_shouldChangeBookCount() {
        books.add(new Book(2, "test book name2", "test publication2", 1));
        given(bookRepository.findByBookId(BOOK_ID)).willReturn(new Book(3, "Test Book Name3", "Test Publication3", 3));
        given(borrowRepository.findByUserId(USER_ID)).willReturn(new User(USER_ID, "test user name", books));
        User user = borrowBookService.borrowBook(USER_ID,3);
        Book book = user.getBorrowBookList().stream().filter(b -> b.getBookId()==3).findFirst().get();
        assertEquals(book.getNoOfCopies(),2);
        assertEquals(user.getBorrowBookList().size(),2);
    }

    @Test
    public void borrowBook_shouldThrowBookNotFoundException() {
        given(bookRepository.findByBookId(NOT_FOUND_BOOK_ID)).willReturn(new Book(NOT_FOUND_BOOK_ID, "test book name2", "test publication2", 0));
        given(borrowRepository.findByUserId(USER_ID)).willReturn(new User(USER_ID, "test user name", books));
